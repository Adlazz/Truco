...
class MazoDeCartas:
    ...
    def comparar_cartas(self, carta_jugador1, carta_computadora):
        valor_carta_jugador1 = self.valor_cartas[carta_jugador1]
        valor_carta_computadora = self.valor_cartas[carta_computadora]

        if valor_carta_jugador1 > valor_carta_computadora:
            return 'Jugador 1'
        elif valor_carta_jugador1 < valor_carta_computadora:
            return 'Computadora'
        else:
            return None
...
class JuegoTruco:
    def __init__(self):
        self.mazo = MazoDeCartas()

    def jugar(self):
        # Repartir cartas intercaladas entre jugador 1 y la computadora
        jugador1_cartas, computadora_cartas = self.mazo.repartir_cartas()

        # Determinar quién es el primero en jugar
        primer_jugador = random.choice(['Jugador 1', 'Computadora'])
        ...
        for ronda in range(3):
            if primer_jugador == 'Jugador 1':
                # Jugar una carta según el turno del jugador
                carta_jugada_jugador1 = self.mazo.jugar_carta(primer_jugador, jugador1_cartas)
                carta_jugada_computadora = self.mazo.jugar_carta('Computadora', computadora_cartas)
            else:
                carta_jugada_computadora = self.mazo.jugar_carta('Computadora', computadora_cartas)
                carta_jugada_jugador1 = self.mazo.jugar_carta(primer_jugador, jugador1_cartas)

            # Comparar cartas y determinar ganador de la ronda
            ganador_ronda = self.mazo.comparar_cartas(carta_jugada_jugador1, carta_jugada_computadora)
        ...
        if puntos_jugador1 > puntos_computadora:
            print("Jugador 1 gana la mano.")
        elif puntos_jugador1 < puntos_computadora:
            print("Computadora gana la mano.")
        else:
            print("Empate en la mano.")

# Crear un objeto juego de truco y jugar
juego_truco = JuegoTruco()
juego_truco.jugar()
